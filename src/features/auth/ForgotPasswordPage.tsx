...
export default function ForgotPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(e?.response?.data?.message ?? "No se pudo enviar el correo de recuperación.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-obsidian flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-xl">
          ...
          <p className="text-lg text-white/50 font-medium leading-relaxed max-w-md">
            Ingresa tu correo para recibir un enlace seguro y restablecer tu acceso al sistema.
          </p>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-6 sm:px-12 lg:px-24 relative">
        <div className="w-full max-w-sm mx-auto">
          ...
          <div className="mb-10 text-center lg:text-left">
            <h2 className="text-2xl font-black text-k-text-h tracking-tight mb-2">Recuperar Acceso</h2>
          ...
          {!success && (
            <form onSubmit={onSubmit} className="space-y-5">
              <div className="space-y-1.5">
                <label htmlFor="reset-email" className="text-[10px] font-bold text-k-text-b uppercase tracking-widest ml-2">Correo Electrónico</label>
                <input
                  id="reset-email"
