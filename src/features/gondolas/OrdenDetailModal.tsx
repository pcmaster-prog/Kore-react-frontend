...
export default function OrdenDetailModal({ orden, onClose, onUpdated }: Props) {
  ...
  const [fullOrden, setFullOrden] = useState<GondolaOrden | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getOrden(orden.id)
      .then(setFullOrden)
      .catch(() => setErr("Error al cargar detalles de la orden"))
      .finally(() => setLoading(false));
  }, [orden.id]);

  const displayOrden = fullOrden ?? orden;

  const statusCfg = STATUS_CONFIG[displayOrden.status] ?? {
    label: displayOrden.status,
        ...
        <div className="relative bg-white rounded-[32px] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-in-up">
          {loading ? (
            <div className="flex flex-col items-center justify-center p-16">
              <div className="h-8 w-8 border-4 border-neutral-200 border-t-obsidian rounded-full animate-spin mb-4" />
              <div className="text-sm font-bold text-neutral-500">Cargando detalles...</div>
            </div>
          ) : (
            <>
              {/* Header */}
              ...
              <div className="overflow-y-auto flex-1 p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    { label: "Empleado", val: displayOrden.empleado.full_name },
                    {
                      label: "Asignada",
                      val: new Date(displayOrden.created_at).toLocaleString("es-MX", {
                        dateStyle: "medium",
                        timeStyle: "short",
                      }),
                    },
                    {
                      label: "Completada",
                      val: displayOrden.completed_at
                        ? new Date(displayOrden.completed_at).toLocaleString("es-MX", {
                            dateStyle: "medium",
                            timeStyle: "short",
                          })
                        : "—",
                ...
                <div>
                  ...
                  {displayOrden.evidencia_url ? (
                    <button
                      onClick={() => setLightboxUrl(displayOrden.evidencia_url!)}
                      className="block rounded-2xl overflow-hidden border border-neutral-100 hover:opacity-90 transition-opacity"
                    >
                      <img
                        src={displayOrden.evidencia_url}
                        alt="Evidencia"
                ...
                {displayOrden.status === "aprobado" && (
                  <div className="rounded-2xl bg-emerald-50 border border-emerald-200 px-5 py-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {displayOrden.approved_at && (
                        <div className="text-xs text-emerald-600 mt-0.5">
                          {new Date(displayOrden.approved_at).toLocaleString("es-MX", {
                            dateStyle: "long",
                            timeStyle: "short",
                          })}
                        </div>
                ...
                {displayOrden.status === "completado" && (
