...
export default function TaskCatalogPanel({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[480px] bg-k-bg-card h-full shadow-2xl flex flex-col animate-in-slide-left pointer-events-auto"
      ...
      >
        ...
        {/* Tab Selection */}
        <div className="px-8 py-4 border-b border-k-border shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
            ...
            <div className="space-y-6 animate-in-fade">
              <div className="space-y-4">
                ...
                {isEnabled("newTaskModal") && newEstMin && selectedEmps.length > 0 && (
                  <div className="rounded-2xl bg-k-badge-p-bg border border-k-border p-4 animate-in-fade flex items-center gap-3">
            ...
            <div className="space-y-6 animate-in-fade">
              ...
              <div className="p-5 bg-k-bg-card border border-k-border rounded-3xl shadow-k-card space-y-4">
                ...
                <div className="flex gap-2">
                  <select
                    className="flex-1 bg-k-bg-card2 border border-k-border rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-obsidian/10 transition-shadow appearance-none cursor-pointer disabled:opacity-50"
                    value={routineId}
                  ...
                  <button
                    ...
                    className="px-4 rounded-2xl bg-neutral-100 text-k-text-h font-bold text-xs uppercase tracking-widest hover:bg-neutral-200 disabled:opacity-40 transition-colors"
                  >
                    Asignar
                  </button>
                </div>
              ...
              {/* Catálogo List */}
              <div className="p-5 bg-k-bg-card border border-k-border rounded-3xl shadow-k-card flex flex-col h-[280px]">
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                  {catalog.length === 0 ? (
                    <div className="h-full flex items-center justify-center text-xs text-k-text-b font-medium pb-4">
                    ...
                    catalog.map((it) => {
                      ...
                      return (
                        <label
                          key={reactKey}
                          className={`flex items-start gap-3 p-3 rounded-2xl border cursor-pointer transition-colors ${selected ? "bg-k-bg-sidebar/5 border-obsidian/20" : "bg-k-bg-card border-k-border hover:border-k-border"}`}
                        >
                          <input
                            type="checkbox"
          ...
          <div className="pt-6 border-t border-k-border">
            <div className="flex items-center justify-between mb-4">
              <span className="block text-[11px] font-bold uppercase tracking-widest text-k-text-b">
                Asignar a...
              </span>
            ...
            <div className="flex flex-wrap gap-2 max-h-[160px] overflow-y-auto custom-scrollbar content-start">
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl border text-xs font-bold transition-all flex items-center gap-2 ${
                      selected
                        ? "bg-k-sb-active-bg text-k-sb-active border-k-border shadow-k-card"
                        : "bg-k-bg-card text-k-text-b border-k-border hover:bg-k-bg-card2"
                    }`}
                  >
                    <div
                      className={`h-2 w-2 rounded-full ${selected ? "bg-k-bar1" : "bg-neutral-300"}`}
                    />
                    {label}
                  </button>
                );
              ...
              {filteredEmpsList.length === 0 && (
                <div className="w-full text-center text-xs text-k-text-b py-2">
        ...
        <div className="p-6 border-t border-k-border bg-k-bg-card2/50 shrink-0">
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-4 bg-k-bg-card border border-k-border rounded-2xl text-[11px] font-black uppercase tracking-widest text-k-text-b hover:text-k-text-h hover:bg-k-bg-card2 hover:border-k-border transition-all shadow-k-card"
            >
              Cancelar
            </button>
            <button
