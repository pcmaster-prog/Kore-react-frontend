...
import { X, Clock, LogIn, ChevronUp, ChevronDown, Plus, Trash2, Save, Loader2 } from "lucide-react";
...
export default function ReglaFormModal({
  ...
  const canSave = useMemo(() => {
    ...
    return !!assigneeId;
  }, [templateIds, dayOfWeek, assigneeType, assigneeId]);
  ...
  const handleSave = () => {
    ...
    if (!canSave) return;
    if (!section) {
      setErr("No hay una sección seleccionada");
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-xl rounded-[32px] bg-k-bg-card shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in-fade animate-in-slide-up border border-k-border">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-8 py-8 custom-scrollbar space-y-6">
          ...
          <div className="space-y-3">
            ...
            {selectedTemplates.length > 0 && (
              <div className="space-y-2">
                {selectedTemplates.map((t, idx) => (
                  <div
                    key={t.id}
                    className="flex items-center gap-2 rounded-full bg-k-bg-card2 border border-k-border px-3 py-1.5"
                  >
                    ...
                    <div className="flex items-center gap-0.5 shrink-0">
                      <button
                        ...
                        disabled={idx === 0}
                        className="h-6 w-6 rounded-full flex items-center justify-center text-k-text-b hover:bg-k-border disabled:opacity-30 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="h-6 w-6 rounded-full flex items-center justify-center text-rose-500 hover:bg-rose-50 transition-colors"
                      >
            ...
            >
              ...
              {showTemplatePicker ? "Ocultar plantillas" : "Agregar plantilla"}
            </button>
          ...
          <div className="space-y-3">
            ...
            {assigneeType === "section_supervisor" && (
              <div className="rounded-2xl bg-k-bg-card2 border border-k-border px-4 py-3 text-sm font-medium text-k-text-b flex items-center gap-2">
                ...
                Se asignará al supervisor de {section.name}
              </div>
            )}
          </div>
