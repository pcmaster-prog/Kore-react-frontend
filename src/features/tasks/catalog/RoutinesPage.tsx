  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-obsidian tracking-tight">Rutinas</h2>
          <p className="text-sm text-neutral-400 mt-1">La mecánica del sistema. Asigna bloques enteros de tareas a tu equipo.</p>
        </div>
        <button 
          onClick={openCreate}
      ...
      <div className="flex items-center justify-between rounded-3xl border border-neutral-100 bg-white p-4 shadow-sm">
        <span className="text-[11px] font-bold tracking-widest uppercase text-neutral-400">Mostrar solo activas</span>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
           {[1, 2, 3].map(i => (
             <div key={i} className="bg-white rounded-3xl border border-neutral-100 p-6 h-[220px]" />
           ))}
        </div>
      ) : items.length === 0 ? (
        <div className="bg-white border border-neutral-100/50 rounded-[40px] p-20 text-center shadow-sm">
          ...
          <h3 className="text-xl font-black text-obsidian tracking-tight mb-2">Sin Rutinas</h3>
          <p className="text-sm font-medium text-neutral-400">Crea rutinas de Apertura o Cierre para estandarizar procesos.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((r) => (
            <div key={r.id} className="bg-white rounded-[32px] p-6 border border-neutral-100 shadow-sm hover:shadow-xl hover:shadow-obsidian/5 hover:border-neutral-200 transition-all flex flex-col group relative overflow-hidden">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div className="flex-1">
                  <h3 className="font-bold text-obsidian line-clamp-2 leading-tight">{r.name}</h3>
                  {r.description && <p className="text-xs text-neutral-400 mt-2 line-clamp-2">{r.description}</p>}
                </div>
                {!r.is_active && (
              ...
              <div className="bg-neutral-50/50 rounded-2xl p-4 mb-4 space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 flex items-center gap-1.5"><Repeat className="h-3 w-3" /> Frecuencia</span>
                  <span className="font-bold text-obsidian capitalize">{r.recurrence}</span>
                </div>
                {r.recurrence === "weekly" && (
                  <div className="flex justify-between items-start text-xs border-t border-neutral-100 pt-3">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 flex items-center gap-1.5 mt-0.5"><Calendar className="h-3 w-3" /> Días</span>
                    <span className="font-medium text-obsidian text-right max-w-[120px]">{formatWeekdays(r.weekdays)}</span>
                  </div>
                )}
              </div>
