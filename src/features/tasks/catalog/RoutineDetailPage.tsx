...
import { Calendar, Trash2, GripVertical, Repeat, ArrowLeft, Plus } from "lucide-react";
...
export default function RoutineDetailPage() {
  ...
  function PriorityBadge({ p }: { p?: string }) {
    if (!p) return <span className="text-[10px] font-bold text-neutral-400">—</span>;
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-obsidian tracking-tight flex items-center gap-3">
              ...
              {routine && (
                <span className={`text-[10px] uppercase font-black tracking-widest px-2 py-1 rounded-md ${routine.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-neutral-100 text-neutral-500'}`}>
            ...
            {routine ? (
              <div className="flex items-center gap-3 text-sm text-neutral-400 mt-1.5 font-medium">
                <span className="flex items-center gap-1.5"><Repeat className="h-3.5 w-3.5" /> <span className="capitalize">{routine.recurrence}</span></span>
                {routine.recurrence === "weekly" && (
                  <>
                    <span className="text-neutral-300">•</span>
                    <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> {formatWeekdays(routine.weekdays)}</span>
                  </>
                )}
              </div>
            ) : null}
      ...
      <div className="bg-white rounded-[40px] border border-neutral-100 shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-neutral-50 flex items-center justify-between gap-3 bg-neutral-50/30">
          <div>
            <h3 className="text-sm font-black text-obsidian tracking-tight">Plantillas de la Rutina</h3>
            <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mt-0.5">Orden de ejecución</p>
          </div>
        ...
        {loading ? (
          <div className="p-16 flex flex-col items-center gap-4 text-neutral-400 bg-neutral-50/50">
           ...
           <div className="p-20 text-center flex flex-col items-center">
             ...
             <h4 className="text-lg font-black text-obsidian tracking-tight">No hay plantillas aquí</h4>
             <p className="text-sm font-medium text-neutral-400 mt-1 max-w-sm">Esta rutina no tiene tareas dentro. Añade plantillas para poder usarla.</p>
           </div>
        ) : (
          <div className="p-4 space-y-2">
            {items
              ...
              .map((it, idx) => {
                ...
                return (
                  <div key={it.id} className="flex items-center gap-4 p-4 rounded-3xl bg-neutral-50 hover:bg-neutral-100 transition-colors border border-neutral-100 group">
                    ...
                    <div className="h-10 w-10 rounded-xl bg-white border border-neutral-200 shadow-sm flex items-center justify-center shrink-0">
                      <span className="text-xs font-black text-obsidian">{idx + 1}</span>
                    </div>

                    <div className="flex-1 min-w-0">
                      <h4 className="text-sm font-bold text-obsidian truncate">{t?.title ?? it.template_id}</h4>
                      {t?.description && (
                         <div className="text-xs text-neutral-500 mt-1 truncate">{t.description}</div>
                      )}
                    </div>
