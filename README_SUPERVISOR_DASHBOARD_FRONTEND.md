# Kore — Dashboard Supervisor: Frontend
...
Modificar `ManagerDashboard.tsx` para mostrar una vista diferente cuando
el rol es `supervisor`. El admin sigue viendo la vista actual sin cambios.

---

## 1. Tipos nuevos

**Agregar en `src/features/tasks/types.ts` o crear `src/features/dashboard/types.ts`:**
...
**Agregar en `src/features/dashboard/api.ts` o en `src/features/tasks/api.ts`:**
...
## 3. Modificar `ManagerDashboard.tsx`

**Archivo:** `src/features/dashboard/ManagerDashboard.tsx`

### 3a. Importar lo necesario

```tsx
import { getSupervisorDashboard } from "./api"; // o desde tasks/api
...
export default function ManagerDashboard() {
  ...
  // Si es supervisor, renderizar vista supervisor
  if (isSupervisor) {
    return <SupervisorDashboard />;
  }

  // Si es admin, renderizar vista existente (sin cambios)
  return <AdminDashboard />;
}
```

Renombrar el contenido actual del componente a `AdminDashboard` (componente interno).

---

## 4. Componente `SupervisorDashboard`

**Crear como componente interno en `ManagerDashboard.tsx`:**

```tsx
function SupervisorDashboard() {
  const { user } = auth.get();
  const userName = user?.name?.split(" ")[0] ?? "Supervisor";
    ...
    <div className="space-y-8 animate-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <WorkloadCard
          ...
          onAssignTask={(taskId, taskTitle) =>
            setAssignModal({ open: true, taskId, taskTitle })
          }
        />
      </div>
...
function WorkloadCard({
  ...
  // Max minutos para la barra de progreso (8 horas = 480 min)
    ...
    <div className="bg-white rounded-[40px] p-8 shadow-sm border border-neutral-100/50">
      ...
      <div className="space-y-3">
        {workload.length === 0 ? (
          ...
          workload
            .sort((a, b) => a.total_minutes - b.total_minutes) // Menos carga primero
            .map((emp) => (
...
## 6. Componente `PendingReviewCard`
...
## 7. Componente `AssignTaskModal`

Modal que aparece cuando el supervisor quiere asignar una tarea.
Muestra la carga de trabajo para ayudar a decidir a quién asignar.
...
## 8. Botón "Asignar tarea" en el dashboard

Agregar en el header del `SupervisorDashboard` junto al saludo:
...
// Loading spinner (reutilizar el existente del dashboard)
...
| Archivo | Acción |
|---|---|
| `src/features/dashboard/ManagerDashboard.tsx` | Modificar — separar en `AdminDashboard` + `SupervisorDashboard` |
| `src/features/dashboard/types.ts` | Crear — tipos para workload y supervisor dash |
| `src/features/dashboard/api.ts` | Modificar — agregar `getSupervisorDashboard()` |
